...
%
This temporal spread allows, albeit with low-resolution, tracking of haplotype groups through time.
%
The spatially widespread F1 group contains haplotypes from samples collected between 2009-2012 (Figure \ref{fig:map}, \cite{clarkson2019genome}), including within just the 'ancestral node' of the median joining network (Figure \ref{fig:networks}).
%
To enable the large geographic spread found, even in this central F1 node (\textit{e.g.} Ghana to Angola), suggests the F1 group existed for some time before the earliest collection in 2009.
%
Haplotype groups F4, F5 and S2, all carry haplotypes from samples collected in Cameroon (2009) and Gabon (2000).
%
With 2009 and 2000 collections found within the same nodes (Figure \ref{fig:networks}), these data demonstrate how, even in these high diversity organisms \cite{Ag1000gConsortium2017}, some nucleotide sequences can persist unchanged for almost a decade.
%%
...
%
It should be noted that, due to the retrospective nature of this study, the detection date of a core haplotype (or haplotype group) does not signify its origin.
%%
